...
import {
  ...
  BarChart3, Trophy, ChevronDown, Activity,
} from "lucide-react";
...
// Extract all unique exercise names across all workouts
...
// For a given exercise, return [{date, maxWeight}] sorted chronologically
...
// PRs: per exercise, highest weight ever
...
function LineChart({ data, nl }: { data: { date: string; weight: number }[]; nl: boolean }) {
  ...
  const area = `M${toX(0)},${toY(data[0].weight)} ` +
    data.map((d, i) => `L${toX(i)},${toY(d.weight)}`).join(" ") +
    ` L${toX(data.length - 1)},${PAD.t + innerH} L${PAD.l},${PAD.t + innerH} Z`;
  ...
  const xLabels = [0, Math.floor((data.length - 1) / 2), data.length - 1].map((i) => ({
    i,
    label: new Date(data[i].date).toLocaleDateString(nl ? "nl-NL" : "en-US", { day: "numeric", month: "short" }),
  }));
    ...
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: 160 }}>
      ...
      {yLabels.map((v) => (
        <text key={v} x={PAD.l - 6} y={toY(v) + 4} textAnchor="end"
          className="fill-gray-500" style={{ fontSize: 11 }}>{v}kg</text>
      ))}
      {/* X labels */}
      {xLabels.map(({ i, label }) => (
        <text key={i} x={toX(i)} y={H - 4} textAnchor="middle"
          className="fill-gray-600" style={{ fontSize: 10 }}>{label}</text>
      ))}
      {/* Area fill */}
      ...
      {/* Dots */}
      {data.map((d, i) => (
        <circle key={i} cx={toX(i)} cy={toY(d.weight)} r={i === prIdx ? 5 : 3}
          fill={i === prIdx ? "#facc15" : "#22c55e"} stroke="#111827" strokeWidth="2" />
      ))}
    </svg>
  );
}
...
export default function DashboardPage() {
  ...
  const progressData = useMemo(
    () => selectedExercise ? getProgressData(workouts, selectedExercise) : [],
  ...
  // Auto-select most trained exercise
  useEffect(() => {
  ...
  const avgDuration = totalWorkouts > 0
    ? Math.round(workouts.reduce((s, w) => s + w.duration, 0) / totalWorkouts) : 0;
  ...
  const ui = {
    title: nl ? "Dashboard" : "Dashboard",
    subtitle: nl ? "Jouw voortgang in één overzicht" : "Your progress at a glance",
    totalWorkouts: nl ? "Trainingen" : "Workouts",
    avgDuration: nl ? "Gem. duur" : "Avg duration",
    thisWeek: nl ? "Deze week" : "This week",
    progress: nl ? "Voortgang per oefening" : "Progress per exercise",
    selectExercise: nl ? "Kies oefening…" : "Choose exercise…",
    pr: nl ? "PR's" : "Personal Records",
    prLabel: nl ? "Persoonlijk record" : "Personal record",
    frequency: nl ? "Trainingsfrequentie" : "Training frequency",
    weeks: nl ? "wk" : "wk",
    sessions: nl ? "sessies" : "sessions",
    muscleBalance: nl ? "Spiergroep-balans" : "Muscle group balance",
    recentWorkouts: nl ? "Recente trainingen" : "Recent workouts",
    noData: nl ? "Log trainingen om statistieken te zien." : "Log workouts to see your stats.",
    noProgress: nl ? "Nog geen data voor deze oefening." : "No data yet for this exercise.",
    minutes: nl ? "min" : "min",
    exercises: nl ? "oefeningen" : "exercises",
  };
  ...
  if (workouts.length === 0) {
    return (
      <div className="sm:ml-52 flex flex-col items-center justify-center py-20 gap-4">
        ...
        <p className="text-gray-400 text-sm">{ui.noData}</p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-500">{ui.totalWorkouts}</span>
          </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-500">{ui.avgDuration}</span>
          </div>
          <p className="text-2xl font-bold">{avgDuration}<span className="text-sm font-normal text-gray-500 ml-1">{ui.minutes}</span></p>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-1">
          <div className="flex items-center gap-2">
            <Calendar className="w-4 h-4 text-blue-400" />
            <span className="text-xs text-gray-500">{ui.thisWeek}</span>
          </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="relative">
            ...
            >
              <span className="truncate">{selectedExercise || ui.selectExercise}</span>
              ...
              <div className="absolute right-0 top-full mt-1 z-30 bg-gray-800 border border-gray-700 rounded-xl shadow-xl overflow-hidden w-56 max-h-64 overflow-y-auto">
                {exerciseNames.map((name) => (
                  <button key={name} onClick={() => { setSelectedExercise(name); setShowExercisePicker(false); }}
                    className={`w-full text-left px-3 py-2.5 text-sm transition-colors border-b border-gray-700/50 last:border-0 ${
                      name === selectedExercise ? "bg-green-600/20 text-green-400" : "text-gray-300 hover:bg-gray-700"
                    }`}>
                    {name}
                  </button>
                ))}
        ...
        {selectedExercise && progressData.length > 0 ? (
          <div>
            ...
            <div className="flex items-center justify-between mt-3 text-xs text-gray-500">
              <span>{progressData.length} {ui.sessions}</span>
              <span className="flex items-center gap-1">
                <span className="w-2.5 h-2.5 rounded-full bg-yellow-400 inline-block" />
                {nl ? "PR" : "PR"}: <span className="font-semibold text-white ml-1">{Math.max(...progressData.map((d) => d.weight))}kg</span>
              </span>
            </div>
          </div>
        ) : (
          <p className="text-sm text-gray-600 py-8 text-center">{ui.noProgress}</p>
        )}
      </div>
            ...
            {prs.map((pr, i) => (
              <div key={pr.name} className="flex items-center gap-3">
                <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                  ...
                  i === 2 ? "bg-orange-700/20 text-orange-400" : "bg-gray-800 text-gray-500"
                }`}>{i + 1}</span>
                ...
                <span className="text-xs text-gray-600 shrink-0">
                  {new Date(pr.date).toLocaleDateString(nl ? "nl-NL" : "en-US", { day: "numeric", month: "short" })}
                </span>
              </div>
            ))}
          </div>
                ...
                <div className="w-full flex items-end" style={{ height: "64px" }}>
                  <div
                    className={`w-full rounded-t-md transition-all ${
                      isCurrentWeek ? "bg-blue-500" : w.count > 0 ? "bg-blue-700" : "bg-gray-800"
                    }`}
                    style={{ height: w.count > 0 ? `${pct}%` : "4px" }}
                  />
                </div>
                <span className="text-xs text-gray-700 text-center leading-tight" style={{ fontSize: 9 }}>
                  {w.label}
                </span>
              </div>
            );
              ...
              return (
                <div key={muscle}>
                  ...
                  <div className="w-full bg-gray-800 rounded-full h-1.5">
                    <div className={`h-1.5 rounded-full transition-all ${MUSCLE_COLORS[muscle] ?? "bg-gray-500"}`}
                      style={{ width: `${pct}%` }} />
                  </div>
                </div>
              );
              ...
              <div key={w.id} className="flex items-center gap-3 py-2 border-b border-gray-800 last:border-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">
                    {w.exercises.length} {ui.exercises} · {w.duration} {ui.minutes}
                  </p>
                </div>
                <span className="text-xs text-gray-600 shrink-0">
                  {new Date(w.date).toLocaleDateString(nl ? "nl-NL" : "en-US", { day: "numeric", month: "short" })}
                </span>
              </div>
            ))}
        </div>
